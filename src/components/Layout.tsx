...
const getRoleLabel = (role: string): string =>
  role === UserRole.ADMIN ? 'Admin' : 'Teammate';
...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems: NavItemType[] = [
    ...
    { to: '/my-tasks', icon: ClipboardList, label: 'My Tasks', section: 'Tasks' as const },
    ...
    ...(isAdmin ? [{ to: '/members', icon: Users, label: 'Members', section: 'Settings' as const }] : []),
  ...
  return (
    <div className="min-h-screen md:h-screen md:overflow-hidden bg-slate-100/80 flex flex-col md:flex-row">
        ...
        <div className="p-3 border-t border-slate-100">
          <div className="my-2 flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 uppercase">{getRoleLabel(user.role)}</p>
            </div>
          </div>
          <button
            onClick={logout}
      ...
      <div className="md:hidden bg-white border-b border-slate-200 px-4 py-3 flex justify-between items-center sticky top-0 z-20 shadow-sm">
        <h1 className="text-lg font-bold text-slate-800">Nerdshouse Tasks</h1>
        <button
