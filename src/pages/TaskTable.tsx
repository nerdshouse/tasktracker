...
export const TaskTable: React.FC = () => {
  ...
  const isAdminMyTasksView = isAdmin && isMyTasksRoute;
  const isSelfTasksView = isTeammate || (isAdmin && isMyTasksRoute);
  ...
  const getActiveFilters = useCallback(() => {
    ...
    return filters;
  }, [user?.id, isSelfTasksView, recurringFilter, resolveDoerDateRange, statusFilter, dateFilter]);
  ...
  }, [getDoerBaseFilters, isAdminMyTasksView, isRecurringMasterTask, sortConfig, sortRowsByConfig, user?.id]);
  ...
  const applyNameFilters = useCallback(
    (list: Task[]) => {
      ...
      return list.filter((task) => {
        const assignee = getUserName(task.assigned_to_id, allUsers).toLowerCase();
        const assigner = getUserName(task.assigned_by_id, allUsers).toLowerCase();
  ...
  const handleExportCsv = async () => {
    if (!isAdmin || isSelfTasksView || exportingCsv) return;
    ...
    try {
      ...
      const columns: CsvColumn<Task>[] = [
        ...
        { header: 'Verifier Name', accessor: (t) => getUserName(t.verifier_id, allUsers) },
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    const isAssigneeLimitedEdit = isTeammate && editingTask.assigned_to_id === user.id;
                  ...
                  >
                    ...
                    <td>
                      <span className="text-sm font-medium text-slate-700 whitespace-pre-wrap">
                        {getUserName(t.assigned_to_id, allUsers)}
                        {t.assignee_deleted && (
                          <span className="ml-2 text-xs px-2 py-0.5 rounded bg-slate-200 text-slate-600">Member deleted</span>
                        )}
                      </span>
                    ...
                    */}
                    ...
                    <td>
                      <span className="text-sm font-medium text-slate-700 whitespace-pre-wrap">
                        {getUserName(t.verifier_id, allUsers) || t.verified_by || (t.verification_required ? 'Required' : '-')}
                      </span>
                    </td>
                    {/* <td className="text-center">
                    ...
                    <td className="py-3 px-2 text-right pr-4">
                      <div className="flex flex-col gap-1 sm:flex-row sm:items-center justify-end py-2 h-full">
                        {(() => {
                          ...
                          const isAssignedByTeammate = assignerUser?.role === UserRole.TEAMMATE;

                          const canEditTask =
                            isAssigner ||
                            (isAdminUser && !isAssignedByTeammate);

                          const canDeleteTask =
                            isAssigner ||
                            (!isSelfTasksView && isAdminUser && !isAssignedByTeammate);

                          const canClosePermanently =
                            t.recurring !== 'none' &&
                            t.status !== 'closed_permanently' &&
                            (isAssigner || isAdminUser);
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="card p-6 max-w-md w-full shadow-xl">
            ...
            <p className="text-sm text-slate-600 mb-3">
              Add a comment for <strong>{getUserName(rejectTask.assigned_to_id, allUsers)}</strong>. They will see it on the task.
            </p>
            <textarea
              value={rejectComment}
                    ...
                    <div>
                      ...
                      {isAssigneeLimitedEdit ? (
                        <p className="h-10 flex items-center text-sm text-slate-800 border border-slate-200 rounded-lg px-3 bg-slate-50">
                          {getUserName(editingTask.assigned_to_id, allUsers)}
                        </p>
                      ) : (
