...
import { Holiday, Absence, UserRole, Client } from '../types';
          ...
          <div className="p-6">
                    ...
                    <table className="w-full text-sm">
                      <thead className="sticky top-0 bg-slate-50 z-10 border-b border-slate-200">
                        <tr>
                          ...
                          {isAdmin && <th className="text-right py-3 px-4 font-semibold text-slate-700">Actions</th>}
                        </tr>
                      </thead>
                      <tbody>
                        {holidays.length === 0 ? (
                          <tr>
                            <td colSpan={isAdmin ? 3 : 2} className="py-8 px-4 text-center text-slate-500">
            ...
            <div className="mt-8 border-t border-slate-100 pt-8">
              ...
              >
                <h3 className="font-semibold text-slate-800">
                  Clients
                  <span className="text-sm font-normal text-slate-500 ml-2">({clients.length})</span>
                </h3>
                {clientsOpen ? <ChevronUp size={20} className="text-slate-400" /> : <ChevronDown size={20} className="text-slate-400" />}
              </button>
              {clientsOpen && (
                <>
                  {isAdmin && (
                    <div className="mb-4">
                      <Button type="button" onClick={() => setShowAddClientModal(true)} size="sm">
                        <Plus size={16} className="mr-1.5" />
                        Add Client
                      </Button>
                    </div>
                  ...
                  >
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {clients.length === 0 ? (
                          <tr>
                            <td colSpan={isAdmin ? 2 : 1} className="py-8 px-4 text-center text-slate-500">
                          ...
                          clients.map((c) => (
                            <tr key={c.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                              ...
                              {isAdmin && (
                                <td className="py-2.5 px-4 text-right">
                                  <Button size="sm" variant="danger" onClick={() => handleDeleteClient(c.id)}>
                                    Delete
                                  </Button>
                                </td>
                              )}
