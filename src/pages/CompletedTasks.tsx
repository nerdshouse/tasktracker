...
import { ChevronsLeft, ChevronLeft, ChevronRight, ChevronsRight, Search, ChevronDown, CheckCircle2 } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { api } from '../services/api';
import { Task, UserRole } from '../types';
import { formatDateDDMMYYYY, getDisplayRecurring, formatRecurringLabel, getUserName, getUserCity, getClientName } from '../lib/utils';
...
export const CompletedTasks: React.FC = () => {
    ...
    }, [isAdmin, isTeammate, user?.id, dateFilter, customStart, customEnd]);
    ...
    }, [tasks, isTeammate, debouncedAssignedTo, debouncedAssignedBy, recurringFilter, taskById]);
    ...
    return (
        <div className="space-y-6">
            ...
            <p className="text-slate-500 text-sm">
                {isAdmin ? 'All tasks that have been successfully completed or permanently closed.' : 'Tasks you have successfully completed or permanently closed.'}
            </p>
                            ...
                            {loading ? (
                                ...
                                pageTasks.map((task) => (
                                    <tr key={task.id} className="border-t border-slate-100">
                                        ...
                                        <td className="px-4 py-3 text-slate-600">{getUserName(task.assigned_to_id, allUsers) || '-'}</td>
                                        <td className="px-4 py-3 text-slate-600">{getUserName(task.assigned_by_id, allUsers) || '-'}</td>
                                        <td className="px-4 py-3 text-slate-600">{getClientName(task.client_id, allClients) || '-'}</td>
                                        <td className="px-4 py-3 text-slate-600">{getUserName(task.verifier_id, allUsers) || task.verified_by || '-'}</td>
