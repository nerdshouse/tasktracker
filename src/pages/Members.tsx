...
export const Members: React.FC = () => {
  ...
  const [newUserRole, setNewUserRole] = useState<UserRole>(UserRole.TEAMMATE);
  ...
  const [editRole, setEditRole] = useState<UserRole>(UserRole.TEAMMATE);
          ...
          for (const row of processedRows) {
            ...
            try {
              const createdUser = await api.createUser({
                ...
                role: UserRole.TEAMMATE, // Enforce Teammate role for bulk imports
  ...
  if (!isAdmin) return <div className="text-slate-500">Access denied. Only Admins can view Members.</div>;
              ...
              <form onSubmit={handleAddMember} className="space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value={UserRole.TEAMMATE}>Teammate</option>
                  </select>
                </div>
                <Input
                  label="City"
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <Button type="button" variant="secondary" onClick={() => { setShowAddForm(false); setError(''); setNewUserName(''); setNewUserEmail(''); setNewUserPassword(''); setNewUserCity(''); setNewUserPhone(''); setNewUserRole(UserRole.TEAMMATE); }}>
                    Cancel
                  </Button>
                </div>
              </form>
            ...
            {users.slice((currentPage - 1) * rowsPerPage, currentPage * rowsPerPage).map((u) => (
              <tr key={u.id} className="border-b border-slate-100 hover:bg-slate-50">
                ...
                <td className="py-3 px-4">
                  <span className="px-2 py-0.5 rounded text-xs font-medium bg-slate-100 text-slate-700 uppercase">
                    {u.role}
                  </span>
                </td>
