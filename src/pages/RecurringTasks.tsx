...
export const RecurringTasks: React.FC = () => {
  ...
  }, [user?.id, isTeammate, recurringFilter, resolveDoerDateRange]);
  ...
  const filteredTasks = useMemo(() => {
    ...
    return allTasks.filter((t) => {
      if (assignedToQuery && !getUserName(t.assigned_to_id, allUsers).toLowerCase().includes(assignedToQuery)) return false;
      if (assignedByQuery && !getUserName(t.assigned_by_id, allUsers).toLowerCase().includes(assignedByQuery)) return false;
      return true;
    });
                  ...
                  <tr key={t.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {(isAdmin || t.assigned_by_id === user?.id) && (
                          <Button
                            variant="secondary"
                            size="sm"
          ...
          <div className="card p-6 max-w-2xl w-full max-h-[85vh] overflow-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="text-slate-700 mt-1">{getUserName(viewTask.assigned_to_id, allUsers) || '-'}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-slate-500">Assigned By</label>
                <p className="text-slate-700 mt-1">{getUserName(viewTask.assigned_by_id, allUsers) || '-'}</p>
              </div>
              {/*
              <div>
              ...
              */}
              <div>
                ...
                <p className="text-slate-700 mt-1">{getUserName(viewTask.verifier_id, allUsers) || (viewTask.verification_required ? 'Required' : '-')}</p>
              </div>
              <div>
