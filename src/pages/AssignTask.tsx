...
import { User, Task, RecurringType, Client, TaskPriority } from '../types';
...
export const AssignTask: React.FC = () => {
  ...
  useEffect(() => {
    api.getUsers().then(setUsers).catch((e) => console.error('Failed to load users:', e));
    api.getHolidays().then(setHolidays).catch((e) => console.error('Failed to load holidays:', e));
    api.getClients().then(setClients).catch((e) => console.error('Failed to load clients:', e));
  }, [user?.role, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (assigneeUser?.phone) {
        try {
          const link = `http://localhost:5173/#/tasks`;
