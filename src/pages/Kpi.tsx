 ...
 * © 2026 Nerdshouse. All rights reserved.
 *
...
export const Kpi: React.FC = () => {
  ...
  useEffect(() => {
    if (isAdmin && !isTeammate && !sortConfig) {
      setSortConfig({ key: 'overdue_percent', direction: 'desc' });
    }
  }, [isTeammate, isAdmin, sortConfig]);
  ...
  useEffect(() => {
    ...
    setLoading(false);
  }, [allData, dateFilter, customStart, customEnd, isAdmin, user?.id]);
    ...
    <div>
      <>
        {/* <p className="text-slate-600 mb-6">
            {isAdmin ? 'Full team KPI.' : 'Your personal KPI.'} Tasks on holidays and during absence are excluded.
          </p> */}
        ...
        <div className={`${isAdmin ? 'max-h-[70vh] overflow-auto' : 'overflow-x-auto'}`}>
          <table className="w-full border-collapse bg-white rounded-xl border border-slate-200 shadow-sm">
            <thead className={isAdmin ? 'sticky top-0 z-20' : undefined}>
            ...
            <tbody>
              {[...memberRows]
                ...
                .sort((a, b) => {
                  const activeSort = sortConfig || (isAdmin && !isTeammate ? { key: 'overdue_percent', direction: 'desc' as const } : null);
                ...
                .map((row) => (
                  <tr
                    ...
                    className={`border-b border-slate-100 hover:bg-slate-50 ${!isTeammate ? 'cursor-pointer' : ''}`}
                  >
                    <td className="py-3 px-4 font-medium text-slate-800">{row.userName}</td>
                    {!isTeammate && <td className="py-3 px-4 text-slate-600">{row.city || '-'}</td>}
                    {!isTeammate && <td className="py-3 px-4 text-center text-slate-700">{row.total_assigned}</td>}
                    {!isTeammate && <td className="py-3 px-4 text-center text-green-600">{row.on_time_completed}</td>}
                    {!isTeammate && <td className="py-3 px-4 text-center text-amber-600">{row.late_completed}</td>}
                    {!isTeammate && <td className="py-3 px-4 text-center text-red-600">{row.overdue_count}</td>}
