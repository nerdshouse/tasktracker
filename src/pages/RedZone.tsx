        ...
        <div className="rounded-xl border-2 border-red-200 overflow-hidden bg-white shadow-sm mt-4">
          ...
          <div className="divide-y divide-red-100">
            {pageItems.map((t) => {
              ...
              const canEdit = isAdmin || t.assigned_by_id === user?.id;
                  ...
                  <Link to={`/tasks?highlight=${t.id}`} className="flex min-w-0 flex-1 items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-600">
                        {!isTeammate ? (
                          <span>
                            <span className="font-medium text-slate-700">Doer:</span> {getUserName(t.assigned_to_id, allUsers)}
                          </span>
                        ) : null}
                        <span>
                        ...
                        <span>
                          <span className="font-medium text-slate-700">Assigned by:</span> {getUserName(t.assigned_by_id, allUsers)}
                        </span>
                        <span>
          ...
          <div className="card p-6 max-w-md w-full shadow-xl">
            ...
            {formatRecurringLabel(getDisplayRecurring(completeTask, taskById), 'Non-recurring') !== 'Non-recurring' && (
              <p className="text-xs text-slate-600 mb-4">
                {(isTeammate && user?.id !== completeTask.assigned_by_id)
            ...
            <div className="flex gap-2 justify-end">
              {completeTask.recurring !== 'none' && (!isTeammate || user?.id === completeTask.assigned_by_id) && (
                <Button
                  variant="danger"
                  onClick={() =>
                    handleComplete(
