 ...
 */
import { Task, Holiday, Absence, KpiMetrics, User } from '../types';

export function getUserName(userId: string | undefined | null, users: { id: string; name: string }[]): string {
  if (!userId) return '';
  return users.find((u) => u.id === userId)?.name || '';
}

export function getUserCity(userId: string | undefined | null, users: { id: string; city?: string }[]): string {
  if (!userId) return '';
