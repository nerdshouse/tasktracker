...
export const api = {
  ...
  // --- Clients ---
  getClients: async (): Promise<{ id: string; name: string; created_at: string; }[]> => {
    const snap = await getDocs(
      query(collection(db, COLLECTIONS.CLIENTS), orderBy('name', 'asc'))
    );
    return snap.docs.map((d) => {
      const data = d.data();
      return {
  ...
  addClient: async (name: string): Promise<{ id: string; name: string; created_at: string; }> => {
    const ref = await addDoc(collection(db, COLLECTIONS.CLIENTS), {
      name,
      created_at: isoToTimestamp(new Date().toISOString()),
    });
    return { id: ref.id, name, created_at: new Date().toISOString() };
  },
